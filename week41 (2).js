...
*/
console.log("Task: A");
const list = [0, 1, 3, 6, 10, 15, 21, 28, 36, 45, 55];
let sum = 0;
...
for (let index = 0; index < fruits.length; index++) {
  if (fruits[index] === "raspberry") {
    console.log(index);
  }
  continue;
}

function findIndexOfValue(value, list) {
...
for (const element of fruits) {
  if (element.charAt(0) === "b") {
    fruitsStartingWithB.push(element);
  }
}
...
let countOfFruitsWithLengthEight = 0;

for (const element of fruits) {
  if (element.length > 8) countOfFruitsWithLengthEight += 1;
}
console.log(countOfFruitsWithLengthEight);
